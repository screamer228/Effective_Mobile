package com.example.effective_mobile.data.offers.mapper
...
class OffersEntityMapper {
    ...
    fun mapOffersEntityList(dto: OffersDTO): List<OfferEntity> {
        val mappedOffers = dto.offersDto.map { offerDto ->
            val imageUrl = when (offerDto.id) {
                1 -> {
                    IMG_URL_1
                }
